...
const AdminCategoriesScreen: React.FC<AdminCategoriesScreenProps> = ({ user, onNavigate, onLogout, categories, onAddCategory, onUpdateCategory, onDeleteCategory, onAddSubCategory, onDeleteSubCategory, notifications }) => {
    ...
    const handleSaveChanges = () => {
        ...
        if (editingCategoryName.trim() && editingCategoryName.trim() !== categoryToManage.name) {
            onUpdateCategory(categoryToManage.id, editingCategoryName.trim());
        }
    ...
    const handleAddSubCategory = () => {
        ...
        onAddSubCategory(categoryToManage.id, newSubCategoryName.trim());
        setNewSubCategoryName('');
    };
                        ...
                        <div>
                            ...
                            <div className="space-y-2 max-h-48 overflow-y-auto pr-2 border rounded p-2 bg-slate-50">
                                {(categoryInModal.subcategories || []).length > 0 ? (
                                    (categoryInModal.subcategories || []).map(sub => (
                                        <div key={sub} className="flex justify-between items-center bg-slate-200 p-2 rounded">
                                            ...
                                            <button onClick={() => onDeleteSubCategory(categoryInModal.id, sub)} className="text-red-500 hover:text-red-700" aria-label={`Delete ${sub}`}>
                             ...
                             <div className="flex space-x-2 mt-3">
                                ...
                                />
                                <button onClick={handleAddSubCategory} className="px-4 py-2 bg-slate-600 text-white rounded hover:bg-slate-700 text-sm font-medium">Add</button>
                            </div>
                        </div>
