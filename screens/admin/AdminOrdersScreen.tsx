...
const AdminOrdersScreen: React.FC<AdminOrdersScreenProps> = ({ user, onNavigate, onLogout, orders, onViewOrder, notifications }) => {
        ...
        <DashboardLayout user={user} onNavigate={onNavigate} onLogout={onLogout} activeView={AppView.ADMIN_ORDERS} notifications={notifications}>
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-semibold text-gray-800">All Orders</h2>
                 <div className="flex space-x-2">
                    <input type="search" placeholder="Search orders..." className="px-3 py-2 border border-gray-300 rounded-md shadow-sm sm:text-sm" />
            ...
            <div className="bg-white shadow-md rounded-lg overflow-hidden">
                ...
                <div className="flex justify-between items-center p-4">
                    <span className="text-sm text-gray-700">Showing 1 to {orders.length} of {orders.length} results</span>
                    <div className="flex space-x-1">
                        <button className="px-3 py-1 border rounded-md text-sm bg-white hover:bg-gray-100">Previous</button>
                        <button className="px-3 py-1 border rounded-md text-sm bg-white hover:bg-gray-100">Next</button>
                    </div>
                </div>
            </div>
        </DashboardLayout>
