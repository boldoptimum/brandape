...
const LoginScreen: React.FC<LoginScreenProps> = ({ onNavigate, onLogin, onNavigateHome }) => {
  ...
  const quickLoginUsers = [
      mockUsers.find(u => u.type === UserType.BUYER)!,
      mockUsers.find(u => u.type === UserType.VENDOR)!,
      mockUsers.find(u => u.type === UserType.SUPPORT_AGENT)!,
      mockUsers.find(u => u.type === UserType.SUPER_ADMIN)!,
  ];
  
  const getUserRole = (type: UserType) => {
      switch(type) {
      ...
      <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
        ...
        <div className="text-sm text-center">
            <p className="text-slate-600">
                ...
                <button type="button" onClick={() => onNavigate(AppView.JOIN)} className="font-semibold text-emerald-600 hover:text-emerald-800">
                    Sign Up
                </button>
            </p>
            <p>
                 <button type="button" onClick={() => onNavigate(AppView.FORGOT_PASSWORD)} className="font-medium text-sm text-slate-600 hover:text-slate-900">Forgot Password?</button>
            </p>
        </div>
      </form>
    </AuthLayout>
