...
import { AppView, User, Dispute, Order } from '../../types';
import DashboardLayout from '../../components/layouts/DashboardLayout';
import ArrowLeftIcon from '../../components/icons/ArrowLeftIcon';
import Modal from '../../components/shared/Modal';

interface SupportDisputeDetailScreenProps {
  user: User;
  ...
  onBack: () => void;
  onAddMessage: (disputeId: string, message: { user: string; text: string; timestamp: string; }) => void;
  onUpdateStatus: (disputeId: string, status: string, resolutionAction?: {type: 'refund', amount?: number} | {type: 'release'}) => void;
  notifications: any[];
}
...
const SupportDisputeDetailScreen: React.FC<SupportDisputeDetailScreenProps> = ({ user, onNavigate, onLogout, dispute, order, onBack, onAddMessage, onUpdateStatus, notifications }) => {
    ...
    const handleSendMessage = () => {
        ...
        onAddMessage(dispute.id, message);
        setNewMessage('');
    };
    ...
    const handleResolve = (resolutionAction: {type: 'refund', amount?: number} | {type: 'release'}) => {
        ...
        onUpdateStatus(dispute.id, 'Resolved', resolutionAction);
                    ...
                    <div className="lg:col-span-2 bg-white shadow-md rounded-lg p-6 flex flex-col h-[75vh]">
                        ...
                        <div className="flex-1 overflow-y-auto space-y-4 pr-4">
                            {dispute.messages.map((message, index) => (
                                <div key={index} className={`flex items-start gap-3 ${message.user === user.name ? 'flex-row-reverse' : ''}`}>
                                    ...
                                    <div className={`p-3 rounded-lg max-w-md ${message.user === user.name ? 'bg-emerald-500 text-white' : 'bg-gray-100'}`}>
                    ...
                    <div className="bg-white shadow-md rounded-lg p-6 h-fit">
                        ...
                        <div className="mt-6">
                            ...
                            <select id="update-status" value={currentStatus} onChange={(e) => setCurrentStatus(e.target.value)} className="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm rounded-md bg-white">
                             ...
                             <button onClick={() => onUpdateStatus(dispute.id, currentStatus)} className="w-full mt-3 px-4 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 text-sm font-medium">
                                Update Case
                            </button>
                        </div>
