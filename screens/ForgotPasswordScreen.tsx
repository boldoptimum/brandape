...
const ForgotPasswordScreen: React.FC<ForgotPasswordScreenProps> = ({ onNavigate, onResetRequest, onNavigateHome }) => {
  ...
  return (
    <AuthLayout onNavigateHome={onNavigateHome}>
      ...
      <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
        ...
        <div className="text-center">
          <button type="button" onClick={() => onNavigate(AppView.LOGIN)} className="font-medium text-sm text-slate-600 hover:text-slate-900">Back to Login</button>
        </div>
      </form>
    </AuthLayout>
  );
