...
interface RegisterScreenProps {
  ...
  onRegister: (email: string, type: UserType, password?: string) => void;
  onNavigateHome: () => void;
}
...
const RegisterScreen: React.FC<RegisterScreenProps> = ({ userType, onNavigate, onRegister, onNavigateHome }) => {
  const [showPassword, setShowPassword] = useState(false);
  const [fullname, setFullname] = useState('');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');

  const title = userType === UserType.VENDOR ? "Register vendor Account!" : "Register Individual Account!";
  const subTitle = "For the purpose of industry regulation, your details are required.";
  
  const handleSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      onRegister(email, userType, password);
  }

  return (
    <AuthLayout onNavigateHome={onNavigateHome}>
      <div className="flex justify-between items-start mb-6">
        <button onClick={() => onNavigate(AppView.JOIN)} className="flex items-center text-sm font-medium text-slate-600 hover:text-slate-900">
