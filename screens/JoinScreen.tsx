...
interface JoinScreenProps {
  ...
  onSetUserType: (type: UserType) => void;
  onNavigateHome: () => void;
}
...
const JoinScreen: React.FC<JoinScreenProps> = ({ onNavigate, onSetUserType, onNavigateHome }) => {
  ...
  return (
    <AuthLayout imageUrl={Images.backgrounds.joinBg} onNavigateHome={onNavigateHome}>
      <div className="text-right mb-8">
        <p className="text-sm text-slate-600">
          Already have an account?{' '}
          <button onClick={() => onNavigate(AppView.LOGIN)} className="font-semibold text-emerald-600 hover:text-emerald-800">
            Sign In
          </button>
        </p>
      </div>
