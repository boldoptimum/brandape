...
interface CompleteProfileIndividualStep3Props {
  onNavigate: (view: AppView) => void;
  onFinish: (name: string) => void;
  onNavigateHome: () => void;
}

const CompleteProfileIndividualStep3: React.FC<CompleteProfileIndividualStep3Props> = ({ onNavigate, onFinish, onNavigateHome }) => {

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // In a real app, user name would come from state managed across steps
    onFinish("Invictus Innocent");
  }

  return (
    <AuthLayout onNavigateHome={onNavigateHome}>
      <div className="flex justify-between items-start mb-6">
        <button onClick={() => onNavigate(AppView.COMPLETE_PROFILE_INDIVIDUAL_STEP2)} className="flex items-center text-sm font-medium text-slate-600 hover:text-slate-900">
