...
const VerificationScreen: React.FC<VerificationScreenProps> = ({ email, userType, onNavigate, onSuccess, onNavigateHome }) => {
  ...
  const handleBack = () => {
    const backView = userType === UserType.VENDOR ? AppView.REGISTER_VENDOR : AppView.REGISTER_INDIVIDUAL;
    onNavigate(backView);
  }
    ...
    <AuthLayout onNavigateHome={onNavigateHome}>
      ...
      <div className="text-center">
        ...
        <p className="mt-2 text-slate-600">
          We have sent a verification code to <span className="font-bold text-slate-800">{email}</span>.
        </p>
        ...
        <div className="mt-6 text-sm">
            <p className="text-slate-600">Didn't receive the verification code? it could take a bit of time.</p>
            {timer > 0 ? (
                <p className="text-slate-500">Request new code in {timer} seconds</p>
            ) : (
                <button onClick={() => setTimer(30)} className="font-semibold text-slate-600 hover:text-slate-900">
                    Resend Code
                </button>
            )}
        </div>
